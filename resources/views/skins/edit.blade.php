...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('skins.update', $skins->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
           ...
           <div class="mb-3">
                <div class="form-group form-action">
                    ...
                    <label>
                        <input type="radio" name="available" value="1" {{ $skins->available ? 'checked' : '' }}>
                        Есть
                    </label><br>
                    <label>
                        <input type="radio" name="available" value="0" {{ !$skins->available ? 'checked' : '' }}>
                        Нет
                    </label>
                </div>
            </div>
            ...
            <div class="mb-3">
                <div class="form-group form-action">
                    ...
                    <input type="text" name="titleRu" class="form-control" value="{{ old('titleRu', $skins->titleRu) }}"
                        required>

                    <label>Заголовок (En)</label>
                    <input type="text" name="titleEn" class="form-control" value="{{ old('titleEn', $skins->titleEn) }}"
                        required>
                </div>
            </div>
            ...
            <div class="form-group form-action">
                <label>цена</label>
                <input type="text" name="priceRu" class="form-control" value="{{ old('priceRu', $skins->priceRu) }}"
                    required>
            </div>
            ...
            <div class="mb-3">
                ...
                @if ($skins->image)
                    <img src="{{ asset('storage/' . $skins->image) }}" alt="image" width="150" class="mt-2">
                @endif
            </div>
            ...
            <div class="mb-3">
                ...
                @if ($skins->images)
                    @foreach (json_decode($skins->images) as $img)
            ...
            <div class="form-group form-action">
                ...
                <input type="text" name="articleRu" class="form-control"
                    value="{{ old('articleRu', $skins->articleRu) }}" required>

                <label>название продукта (En)</label>
                <input type="text" name="articleEn" class="form-control"
                    value="{{ old('articleEn', $skins->articleEn) }}" required>
            </div>

            <div class="form-check form-action">
                <input class="form-check-input" type="checkbox" name="shape1" id="Shape1"
                    {{ old('shape1', $skins->shape1) ? 'checked' : '' }}>
                <label class="form-check-label" for="Shape1">Право</label>
            </div>

            <div class="form-check form-action">
                <input class="form-check-input" type="checkbox" name="shape2" id="Shape2"
                    {{ old('shape2', $skins->shape2) ? 'checked' : '' }}>
                <label class="form-check-label" for="Shape2">Лево</label>
            </div>

            <div class="mb-3">
                <label for="annotationRu" class="form-action">Аннотация (RU)</label>
                <textarea name="annotationRu" class="form-control" rows="4" required>{{ old('annotationRu', $skins->annotationRu) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="annotationEn" class="form-action">Аннотация (EN)</label>
                <textarea name="annotationEn" class="form-control" rows="4" required>{{ old('annotationEn', $skins->annotationEn) }}</textarea>
            </div>


            <div class="form-group form-action">
                <label>Описание (RU)</label>
                <textarea name="descriptionRu" id="mySummernote" class="form-control" required>{{ old('descriptionRu', $skins->descriptionRu) }}</textarea>

                <label>Описание (En)</label>
                <textarea name="descriptionEn" id="mySummernote1" class="form-control" required>{{ old('descriptionEn', $skins->descriptionEn) }}</textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex gap-2">
                 ...
                 <select name="sizeRu[]" id="size-select" class="form-select mt-2" multiple>
                    @foreach (json_decode($skins->sizeRu, true) ?? [] as $size)
            ...
            <div class="mb-3">
                ...
                <textarea name="informationRu" class="form-control" id="mySummernote2" rows="4" required>{{ old('informationRu', $skins->informationRu) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="informationEn" class="form-action">информация (EN)</label>
                <textarea name="informationEn" class="form-control" rows="4" id="mySummernote3" required>{{ old('informationEn', $skins->informationEn) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="payRu" class="form-action">оплата (RU)</label>
                <textarea name="payRu" class="form-control" id="mySummernote4" rows="4" required>{{ old('payRu', $skins->payRu) }}</textarea>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="DeliveryRu" class="form-control" id="mySummernote6" rows="4" required>{{ old('DeliveryRu', $skins->DeliveryRu) }}</textarea>
            </div>

            <div class="mb-3">
                <label for="DeliveryEn" class="form-action">Доставка (EN)</label>
                <textarea name="DeliveryEn" class="form-control" rows="4" id="mySummernote7" required>{{ old('DeliveryEn', $skins->DeliveryEn) }}</textarea>
            </div>
